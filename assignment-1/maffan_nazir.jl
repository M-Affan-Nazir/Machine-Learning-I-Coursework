...
Use the below let blocks to complete question 1(bcde). You will be graded on your written work, not on the code in these cells. Here we have given you one example of how you might call the above functions, to avoid getting hung up on Julia syntax.
"""
...
let # 1b
	numSamples = 10
	gd = GaussianDistribution(0, 1^0.5)
	data = sample(gd,numSamples)
	sampleMean = mean(data)
	var = var([0.3311,0.1262,-0.1241,-0.2451,-0.4227])
...
let # 1d, 1e
	numSamples = 30
	gd = GaussianDistribution(0, 10^0.5)
	data = sample(gd,numSamples)
	sampleMean = mean(data)
	println(data)
	println(sampleMean)
...
# ╔═╡ db4c8db1-5da1-4ab7-a9e0-e0fb2258cdaa
print(stddev([-4.592670947052854, -2.8318547116664052, 3.4468987961644, -0.2958339642547402, -1.7234698034389602, 2.3832362102866074, -1.244778640067107, -4.72957702356859, 3.9597397226064857, -0.483774104523572, 3.7464120319267553, 2.8601607752328886, -0.10094139546522006, 1.8899278207216252, -2.309496683693811, 0.6056781954819382, -1.2316527615030877, -4.379580438620858, -2.323624033803355, 1.6742926039881851, -0.23083488547999395, 3.904081841558202, 7.1323224864972925, 3.3130502850254517, -1.9646381087733342, 0.5498036950833015, 5.771652923834651, 2.3697600335267848, 4.891566299695061, 3.125471659783496]))
...
In this section we help you plot samples from the Gaussian, to get a better intuition for the impact of the underlying mean, variance and the number of samples. There are no explicit questions related to this section, but it might help you better understand your answers to question 1.
