...
begin
	...
	This section loads and installs all the packages.
	"""
end
...
md"""
In this section you will be implementing gradient descent regressors, assuming $$p(y | x)$$ is Gaussian. First we will create a Gaussian regressor, and then use this to build new regressors.

All the regressors will have:
...
begin
	md"""
	The simple stochastic regressor will be implemented via the stochastic gradient descent rule
	...
	We start from an initial $$w_0 = 0$$ and do multiple epochs over the dataset. Each epoch corresponds to iterating once over the entire dataset, in a random order. Here you only have to implement the function that does one epoch. It will be called multiple times in `train!`, which is a function provided for you described in the section on Training the Models. 
...
function epoch!(reg::SimpleStochasticRegressor, 
	...
	#### BEGIN SOLUTION
	shuffledIndices = shuffle(1:length(X))
	X[:] = X[shuffledIndices]
	Y[:] = Y[shuffledIndices]

	for i in 1:length(X)
		predicted = X[i] * reg.w
		newWeight = reg.w - reg.η * (predicted - Y[i]) * X[i]
		reg.w = newWeight
	end
...
begin
	md"## (b) Evaluating SGD with a Mini-batch Size of 1"
end
...
We will be looking at the `height_weight.csv` dataset found in the data directory. This dataset provides three features `[sex, height, weight]`. In the following regression task we will be using `height` to predict `weight`, ignoring the `sex` feature. You do not need to implement anything in this section.
...
The following functions are defined as utilities to train and evaluate our models. While hidden below, you can expand these blocks to uncover what is happening. `run_experiment` is the main function used to train and evaluate our models.
...
md"""
Now we can finally run the experiment. We set the stepsize for `SimpleStochasticRegressor` to 0.01 and the number of epochs to 30. The below boxplot shows the squared errors for predictions on the test set. 

**To run these experiments and draw plots use** $(@bind __run_qb_experiments PlutoUI.CheckBox())
"""
...
md"""
The results_qb dictionary contains the errors for each of the three models, with keys `"Mean"`, `"Random"` and `"SimpleStochastic"`. You can use the below code to print out the mean and standard deviation of the test error for each model. 

These values are to be reported and compared in your written answer for question (b).

"""

# ╔═╡ debd1501-2fae-431c-85cf-bea549a4bba4
begin
	md"## SGD with one Batch"
end
...
md"""
Now we implement the general SGD rule with minibatches of size greater than 1, but smaller than the size of the dataset. To be extra clear about this, we call it `MiniBatchRegressor`. The update rule for a minibatch `j` with indices for a batch defined by the set  $$\mathcal{I}_j$$ is
...
function epoch!(reg::MiniBatchRegressor,
		        X::AbstractVector{Float64},
		        Y::AbstractVector{Float64})
	# Your code here is for question (c)
	# You will need to move to the experiment section to test your implementation
	#### BEGIN SOLUTION
	for i in 1:reg.b:length(X)
				startIndex = i
				endIndex = min(i+reg.b-1, length(X))
				batchX = X[startIndex:endIndex]
				batchY = Y[startIndex:endIndex]

				gradient = sum((reg.w .* batchX .- batchY) .* batchX) / length(batchX)
				newWeight = reg.w - (reg.η * gradient)
				reg.w = newWeight
	end
...
# This uses the adaptive step size heuristic
...
function epoch!(reg::MiniBatchHeuristicRegressor,
		        X::AbstractVector{Float64},
		        Y::AbstractVector{Float64})
	# Your code here is for question (d)
	# It should look very similar to what you did in (c), except now you need to set the stepsize η using the heuristic we described above.
	#### BEGIN SOLUTION
	for i in 1:reg.b:length(X)
				startIndex = i
				endIndex = min(i+reg.b-1, length(X))
				...
				gradient = sum((reg.w .* batchX .- batchY) .* batchX) / length(batchX)
				gradientSquare = gradient^2
				stepSize = (1+(gradientSquare^0.5))^(-1)
				newWeight = reg.w - (stepSize * gradient)
				reg.w = newWeight
	end
...
begin
	md"## (a) $(_check_complete(__check_stoch_reg)) SGD with a Mini-batch Size of 1"
end
...
begin
	results_qb = run_experiment(regressors_qb, 30)
end

# ╔═╡ ea7a7a69-4953-45a9-ae9e-f25bb71b2349
begin

	if __run_qb_experiments
		__results_qb_checks = Dict{String, Union{Vector{Bool}, Bool}}(
		...
		algs_qb = [
			...
			"SimpleStochastic"]
		for k in algs_qb
			if k in keys(results_qb)
				if k == "Stochastic"
					a *= "- ✅ `$(k)`: "
					__results_qb_checks[k] = results_qb[k].regressor.η == __s_η
					a *= """with stepsize= `$(results_qb[k].regressor.η)` $(results_qb[k].regressor.η == __s_η ? "✅" : "❌" )"""
				else
					a *= "- ✅ `$(k)`"
				end
			else
...
let
	if __run_qb_experiments
		# Play with data here! You can explore how to get different values. The line below only prints the mean test error for the RandomRegressor.
		println(std(results_qb["Mean"].test_error)) 
	end
end
...
begin
	md"## (c) $(_check_complete(__check_mb)) SGD with a MiniBatch Size Greater than 1 "
end
...
md"""
## (d) $(_check_complete(__check_bhr_mini)) Stepsize Heuristic 

In this part of the code, you will implement an adaptive stepsize heuristic (discussed in section 4.4 of the notes) in `epoch!` when the `MiniBatchHeuristicRegressor` is used. The adaptive stepsize $$\eta_t$$ is calculated as follows

```math
\begin{align*}
\eta_{t}= \left(1+ |g_t|\right)^{-1}
\end{align*}
```
where $$g_t$$ is the gradient in the update $$w_{t+1} = w_t - \eta_t g_t$$.

Below we provide the implementation of this stepsize heuristic in `epoch!` when the `BatchHeuristicRegressor` is used.
"""
...
let
	if __run_qb_experiments
		...
		ms = filter((k)->k ∉ ["None"], keys(results_qb))
		p_qb = boxplot(
			collect(ms), 
			[getindex(results_qb[m], :test_error) for m in ms], 
			height=400,
			yaxis=attr(title="Test Error"))
		
		PlotlyJS.relayout(p_qb, height=400, showlegend=false)
	end
end
...
html"""<h1 id="experiment"> (e) Experiments </h1>"""
...
These values are to be reported and compared in your written answer for question (e).
"""
...
let
	if __run_q3_experiments
		...
		println(std(results["MinibatchHeuristic"].test_error))
