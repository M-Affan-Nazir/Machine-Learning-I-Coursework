...
function get_features(pm::Polynomial2Model, _X::AbstractMatrix)
	...
	#Adding x^2:
	for i in 1:N
		startIndex = (num_features - d)-1
		for j in 1:d
			Φ[i,startIndex] = (X[i,j])^2
	...
	#Adding coresponding Terms:
	for i in 1:N
		toInsert = (2*d)+1
		for j in 1:(d-1)
			for k in (j+1):d
				Φ[i,toInsert] = X[i,j] * X[i,k]
				toInsert = toInsert+1
			end
		end
	end
...
function loss(lm::AbstractModel, mse::MSE, X, Y)
	0.0
	#### BEGIN SOLUTION
	yPred = predict(lm,X)[:,1]
	sum = 0
	for i in 1:length(yPred)
		sum = sum + (yPred[i]-Y[i])^2
		
	end
	sum = (1/(2*length(Y)))*sum
	return sum
...
function update!(lm::LinearModel, 
	...
	#### BEGIN SOLUTION
	opt.gbar = opt.gbar .+ ((g).^2)
	t = sqrt.(opt.gbar .+ opt.ϵ)
	t = opt.η ./ t
	t = t .* g
	t = lm.w - t
	lm.w = t
...
function epoch!(mbgd::MiniBatchSGD, model::LinearModel, lossfunc, opt, X, Y)
	...
	for i in 1:mbgd.b:length(Y)
		startIndex = i
		endIndex = min(i+mbgd.b-1,length(Y))
		batchX = X_shuffled[startIndex:endIndex,:]
		batchY = Y_shuffled[startIndex:endIndex]
		update!(model,lossfunc,opt,batchX,batchY)
	end
...
begin
	...
	# HTML(str)
	md"### (b) $(_check_complete(__check_MBGD)) Mini-batch Gradient Descent"
end
...
Below is the helper function for running an experiment.
...
md"""
The following experiment uses the addmistions dataset, which you should report. **You can get the average error and standard error to report from the plot or from below the plot**.
"""
...
md"""
The methods all look similar, because we nicely normalized the data and only used polynonmials of degree 2. This means each dimension has a similar scale or curvature, and so the vector stepsize is not that critical. Let us re-run this experiment, now with unnormalized data.

In the unnormalized data we have also divided all the data points by 1000 to make the learning more stable (*Note: dividing by 1000 is not the same as normalization).

We print out the original ranges, so you can see how different these ranges are across inputs.
